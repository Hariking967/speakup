...
import {z} from 'zod';
import { eq, and, ilike, sql, getTableColumns, desc, count, inArray } from 'drizzle-orm';
...
export const meetingsRouter = createTRPCRouter({
            ...
            const transcriptWithSpeakers = transcript.map((item) => {
                ...
                if (!speaker){
                    return {
                        ...
                        user: {
                            name: "Unkown",
                            image: generateAvatarUri({seed: "Unkown", variant: "botttsNeutral"})
                        }
                    }
                }
                return {
    ...
    getOne: protectedProcedure.input(z.object({id: z.string()}))
        .query(async ({input, ctx}) => {
            const [thisMeeting] = await db.select({
                                        ...getTableColumns(meetings),
                                        agent: agents,
                                        duration: sql<number>`EXTRACT(EPOCH FROM (ended_at - started_at))`.as("duration")
                                        })
    ...
    create: protectedProcedure
            ...
            .mutation( async ({input, ctx}) => {
                const [newMeeting] = await db.insert(meetings).values({...input, userId: ctx.auth.user.id}).returning();
